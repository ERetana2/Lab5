...
#return location of key, or where it should be if not found
def location(h,k):
    b, index = -1, None
    b = h.h(k)
    for i in range(len(h.bucket[b])):
        if h.bucket[b][i].key == k:
            ...
            return b,index
    return b, index
# change the key value k to new_k and rearrange data
def change_key(h,k,new_k):
    i = h.h(k)
    for j in range(len(h.bucket[i])):
        if h.bucket[i][j].key == k:
            currValue = h.retrieve(h.bucket[i][j].key)
            h.delete(h.bucket[i][j].key)
            h.insert(new_k,currValue)
            break
    return 
      # returns a list containing each of the items in the list without repetition  
def unique_items(L):
...
#    bucket 4: [ [14, Hernandez] ]
    ...
    print('\nQuestion 1')
    print(location(h,31))  # (3, 0)
    print(location(h,14))  # (0, 1)
